...
CREATE TABLE IF NOT EXISTS TRECHO (
  ...
  CONSTRAINT fk_TRECHO_AERONAVE1
	...
	WHERE %s in (t.VOO_Id_voo, v.Id_voo)
	AND t.Data_hora BETWEEN %s AND %s;
      
      -- 2
       SELECT A.*  FROM VOO A
	JOIN TRECHO D ON A.ID_VOO = D.VOO_ID_VOO
	JOIN RESERVA B ON D.CODIGO_RESERVA = B.CODIGO_RESERVA
	JOIN CLIENTE C ON B.CLIENTE_CPF = C.CPF
	WHERE LOWER(C.NOME) LIKE LOWER(%s);
	...
	WHERE OPERADORA_CARTAO = %s
	AND Data_pagamento BETWEEN DATE_SUB(CURDATE(), INTERVAL 30 DAY) AND CURDATE();
	 ...
	 AND YEAR(B.DATA_HORA) = YEAR(NOW())-1
